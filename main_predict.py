...
def predict(batch):
    ...
    imgs_list_tst=[np.load(fs.img_list(p, 'ED_ES_adapted')) for p in range(cg.num_partitions)]
    segs_list_tst=[np.load(fs.seg_list(p, 'ED_ES_adapted')) for p in range(cg.num_partitions)]
    ...
    for img, seg in zip(imgs_list_tst, segs_list_tst):
      ...
      adapted_already = 1,#cg.adapted_already,
      ),
      verbose = 1,
      steps = 1,)
                               
      # save u_net segmentation
      time = ff.find_timeframe(seg,1)
      u_gt_nii = nb.load(os.path.join(cg.base_dir,patient_class,patient_id,'seg-nii-0.625',str(time)+'.nii.gz'))
      ...
      save_file = os.path.join(cg.base_dir,patient_class,patient_id,cg.pred_dir,seg_filename + str(time) + '.nii.gz')
